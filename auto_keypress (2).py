...
def wait_for_trigger_keyboard(trigger_key):
    ...
    # Wait for the key to be physically released before continuing,
    # preventing the same keydown from firing the next row instantly
    try:
        keyboard.wait(trigger_key, suppress=False, trigger_on_release=True)
    except Exception:
        pass
    time.sleep(0.3)  # drain any OS key-repeat events
