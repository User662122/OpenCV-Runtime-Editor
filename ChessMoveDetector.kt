...
fun detectPiecesOnBoard(grayBoard: Mat, files: String, ranks: String, cellSize: Int): Pair<List<String>, List<String>> {
    ...
    for (r in 0 until 8) {
        for (c in 0 until 8) {
            ...
            // ✅ FIX: Use absolute brightness, not relative difference
            ...
            // ✅ FIX: Improved color detection logic
            // White pieces: High absolute brightness (>140) OR bright relative to background
            // Black pieces: Low absolute brightness (<110) AND darker than background
            
            if (edgeCount >= curEdgeThresh && !isSmallBrightSpot) {
                pieceDetected = true
                // Use absolute brightness for color determination when edges are present
                colorIsWhite = absBrightness > 130
            } else {
                // Brightness-based detection
                if (absBrightness > 140 || (diff >= curPosThresh && !isSmallBrightSpot)) {
                    if (innerStd in MIN_WHITE_STD..MAX_EMPTY_STD) {
                        pieceDetected = true
                        colorIsWhite = true
                    }
                } else if (absBrightness < 110 && diff <= curNegThresh) {
...
fun getBoardStateFromBitmap(bitmap: Bitmap, boardName: String): BoardState? {
    ...
    // ✅ FIX: Improved orientation detection
    // Check multiple squares to determine orientation more reliably
    val cornerSquares = listOf(
        Pair(0, 0), Pair(0, 7), Pair(7, 0), Pair(7, 7)
    )
    
    val squareBrightnesses = mutableListOf<Pair<Int, Double>>()
    
    for ((row, col) in cornerSquares) {
        val square = boardWarped.submat(row * cellSize, (row + 1) * cellSize, 
                                        col * cellSize, (col + 1) * cellSize)
        val gray = Mat()
        Imgproc.cvtColor(square, gray, Imgproc.COLOR_BGR2GRAY)
        val brightness = Core.mean(gray).`val`[0]
        squareBrightnesses.add(Pair(row * 8 + col, brightness))
    }
    
    // Check if there are pieces in the bottom rows vs top rows
    val bottomRowsBrightness = mutableListOf<Double>()
    val topRowsBrightness = mutableListOf<Double>()
    ...
    for (col in 0 until 8) {
        ...
        for (row in 6..7) {
            ...
            val gray = Mat()
            Imgproc.cvtColor(square, gray, Imgproc.COLOR_BGR2GRAY)
            bottomRowsBrightness.add(Core.mean(gray).`val`[0])
        }
        ...
        for (row in 0..1) {
            ...
            val gray = Mat()
            Imgproc.cvtColor(square, gray, Imgproc.COLOR_BGR2GRAY)
            topRowsBrightness.add(Core.mean(gray).`val`[0])
        }
    }
    
    val avgBottom = bottomRowsBrightness.average()
    val avgTop = topRowsBrightness.average()
    
    // White pieces are brighter, so if bottom is brighter, white is on bottom
    val whiteOnBottom = avgBottom > avgTop
